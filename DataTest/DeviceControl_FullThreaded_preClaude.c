...
int CVICALLBACK AdcConfigureCB (int p, int c, int ev, void *cbd, int e1, int e2)
{
	...
	adRange   = (rangeIdx == 0) ? AD_B_1_V : AD_B_0_2_V;
    currentAdcVoltageLimit = (rangeIdx == 0) ? 1.0 : 0.2;
    ...
    if (err != NoError) { SetCtrlVal(adcTabHandle, TABPANEL_2_ADC_MSG_STATUS, "AI_Config Failed"); return 0; }
    ...
    /* RESTORED: Exact hardware trigger parameters from the legacy working branch */
    err = WD_AI_Trig_Config (adcCard, WD_AI_TRGMOD_POST, trigSrc, trigPol, 0, 0.0, 1, 0, 0, 1); 
    if (err != NoError) { SetCtrlVal(adcTabHandle, TABPANEL_2_ADC_MSG_STATUS, "Trig_Config Failed"); return 0; }

    if (pDiv <= 0) pDiv = 1152; 
    halfBufferSize = 512 * pDiv * ADC_NUM_CHANNELS; 
    adcTotalSamples = halfBufferSize * 2;

    if (adcBuffer) WD_Buffer_Free(adcCard, adcBuffer);
    if (userBuffer) free(userBuffer);
    ...
    if (adcBuffer == NULL || userBuffer == NULL) { return 0; }
    ...
    SetCtrlVal (adcTabHandle, TABPANEL_2_ADC_MSG_STATUS, "Config OK");
...
int CVICALLBACK AdcStartCB (int p, int c, int ev, void *cbd, int e1, int e2)
{
    ...
    /* CRITICAL FIX: Double-buffering requires the explicit buffer size, not 0 */
    totalScans = adcTotalSamples / ADC_NUM_CHANNELS;
    err = WD_AI_ContReadMultiChannels(adcCard, ADC_NUM_CHANNELS, chans, adcBufId, totalScans, 1, 1, ASYNCH_OP);
    
    if (err == NoError) {
        char statusMsg[128];
        sprintf(statusMsg, "Acquisition Active | Recording: %s", recording ? "ON" : "OFF");
        SetCtrlVal(adcTabHandle, TABPANEL_2_ADC_MSG_STATUS, statusMsg);
        ...
        adcRunning = 0;
        char errMsg[128];
        sprintf(errMsg, "Hardware Start Failed: Error %d", err);
        SetCtrlVal(adcTabHandle, TABPANEL_2_ADC_MSG_STATUS, errMsg);
    }
    return 0;
}
...
int CVICALLBACK AdcSingleShotCB (int p, int c, int ev, void *cbd, int e1, int e2)
{
    ...
    /* 6. Restore Gapless Streaming Trigger Configuration */
    WD_AI_Trig_Config(adcCard, WD_AI_TRGMOD_POST, WD_AI_TRGSRC_ExtD, WD_AI_TrgPositive, 0, 0.0, 0, 0, 0, 0);
