...
static double       currentAdcVoltageLimit = 1.0; /* 1.0V or 0.2V based on UI */
/* Diagnosis */
...
void CVICALLBACK ADC_PlotFFT_Deferred (void *callbackData)
{
    ...
    /* 1. Time-Domain Oscilloscope Extraction & Voltage Scaling */
    ...
    PlotY (adcTabHandle, TABPANEL_2_ADC_GRAPH_TIME, timeDataCH0, plotSamples, 
           VAL_DOUBLE, VAL_THIN_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_YELLOW);
    PlotY (adcTabHandle, TABPANEL_2_ADC_GRAPH_TIME, timeDataCH1, plotSamples, 
           VAL_DOUBLE, VAL_THIN_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_CYAN);

    /* 2. Standard FFT Computation */
    ...
    PlotY (adcTabHandle, TABPANEL_2_ADC_GRAPH_FFT, fftMagCH0, plotN, VAL_DOUBLE,
           VAL_THIN_LINE, VAL_NO_POINT, VAL_SOLID, 1, VAL_YELLOW);
    PlotY (adcTabHandle, TABPANEL_2_ADC_GRAPH_FFT, fftMagCH1, plotN, VAL_DOUBLE,
...
int CVICALLBACK AdcConfigureCB (int p, int c, int ev, void *cbd, int e1, int e2)
{
    ...
    /* ---- Step 1: AI Config ---- */
    err = WD_AI_Config (adcCard, timeBase, FALSE, WD_AI_ADCONVSRC_TimePacer, FALSE, TRUE);
    if (err != NoError) {
    ...
    /* ---- Step 4: Trigger config with CORRECT buffer-derived values ---- */
    {
        U32 totalScansPerCh = adcTotalSamples / ADC_NUM_CHANNELS;
        err = WD_AI_Trig_Config (adcCard, WD_AI_TRGMOD_POST, trigSrc, trigPol,
                                 0, 0.0, totalScansPerCh, 0, 0, 0);
        if (err != NoError) {
            sprintf(diagMsg, "[4] Trig_Config FAILED: %d (postTrigScans=%lu, reTrgCnt=0)",
    ...
    /* ---- Step 6: Setup continuous buffer ---- */
    err = WD_AI_ContBufferSetup(adcCard, adcBuffer, adcTotalSamples, &adcBufId);
    if (err != NoError) {
        sprintf(diagMsg, "[6] ContBufferSetup FAILED: %d (count=%lu, bufId=%u)",
                err, (unsigned long)adcTotalSamples, adcBufId);
        SetCtrlVal(adcTabHandle, TABPANEL_2_ADC_MSG_STATUS, diagMsg);
        return 0;
    }
...
int CVICALLBACK AdcStartCB (int p, int c, int ev, void *cbd, int e1, int e2)
{
    ...
    /* ---- Step 1: Reset and re-setup buffer ---- */
    WD_AI_ContBufferReset(adcCard);
    err = WD_AI_ContBufferSetup(adcCard, adcBuffer, adcTotalSamples, &adcBufId);
    if (err != NoError) {
        sprintf(diagMsg, "[S1] ContBufferSetup FAILED: %d", err);
        SetCtrlVal(adcTabHandle, TABPANEL_2_ADC_MSG_STATUS, diagMsg);
        adcRunning = 0;
        return 0;
    }
    ...
    /* ---- Step 2: Trigger config ----
       postTrigScans = total scans per channel (full buffer).
       reTrgCnt = 0: in double-buffer mode the buffer is circular. */
    err = WD_AI_Trig_Config(adcCard, WD_AI_TRGMOD_POST, WD_AI_TRGSRC_ExtD,
                             WD_AI_TrgPositive, 0, 0.0,
                             totalScansPerCh, 0, 0, 0);
    if (err != NoError) {
        sprintf(diagMsg, "[S2] Trig_Config FAILED: %d (postTrig=%lu, reTrg=0)",
                err, (unsigned long)totalScansPerCh);
        SetCtrlVal(adcTabHandle, TABPANEL_2_ADC_MSG_STATUS, diagMsg);
        adcRunning = 0;
        return 0;
    ...
    /* ---- Step 3: Enable double-buffer mode ---- */
    /*err = WD_AI_AsyncDblBufferMode(adcCard, 1);*/
	err = WD_AI_Set_Mode(adcCard, DualBufEn, 0);
    if (err != NoError) {
        sprintf(diagMsg, "[S3] AsyncDblBufferMode(1) returned %d — trying without it", err);
        SetCtrlVal(adcTabHandle, TABPANEL_2_ADC_MSG_STATUS, diagMsg);
        /* Continue — some driver versions handle this implicitly */
    }

    /* ---- Step 4: Register the half-buffer callback ---- */
    err = WD_AI_EventCallBack(adcCard, 1, DBEvent, (U32)DBEvent_Callback);
    if (err != NoError) {
        sprintf(diagMsg, "[S4] EventCallBack FAILED: %d", err);
        SetCtrlVal(adcTabHandle, TABPANEL_2_ADC_MSG_STATUS, diagMsg);
        adcRunning = 0;
        return 0;
    }

    /* ---- Step 5: Start recording thread if needed ---- */
    if (recording) {
    ...
    /* ---- Step 6: Start acquisition ----
       Try ContScanChannels first (simpler path for PCI-9846H simultaneous sampling),
       fall back to ContReadMultiChannels if not supported. */
    err = WD_AI_ContScanChannels(adcCard, 1, adcBufId, totalScansPerCh, 1, 1, ASYNCH_OP);

    if (err != NoError) {
        sprintf(diagMsg, "[S6a] ContScanChannels err=%d — trying ContReadMultiChannels", err);
        SetCtrlVal(adcTabHandle, TABPANEL_2_ADC_MSG_STATUS, diagMsg);

        /* Fallback: ContReadMultiChannels */
        err = WD_AI_ContReadMultiChannels(adcCard, ADC_NUM_CHANNELS, chans, adcBufId,
                                          totalScansPerCh, 1, 1, ASYNCH_OP);
    }

    if (err == NoError) {
        sprintf(diagMsg, "Acq Active | Rec:%s | scans/ch=%lu | bufSamp=%lu | pDiv=%d",
                recording ? "ON" : "OFF", (unsigned long)totalScansPerCh,
                (unsigned long)adcTotalSamples, currentProgDiv);
        SetCtrlVal(adcTabHandle, TABPANEL_2_ADC_MSG_STATUS, diagMsg);
        ...
        adcRunning = 0;
        sprintf(diagMsg, "[S6] BOTH ContScan and ContReadMulti FAILED: %d | "
                "scans/ch=%lu | bufSamp=%lu | halfBuf=%lu | bufId=%u",
                err, (unsigned long)totalScansPerCh,
                (unsigned long)adcTotalSamples, (unsigned long)halfBufferSize, adcBufId);
        SetCtrlVal(adcTabHandle, TABPANEL_2_ADC_MSG_STATUS, diagMsg);
    }
    return 0;
}
...
int CVICALLBACK AdcSingleShotCB (int p, int c, int ev, void *cbd, int e1, int e2)
{
    ...
    /* 3. Temporarily enforce finite hardware trigger (postTrigScans = totalScans) */
    err = WD_AI_Trig_Config(adcCard, WD_AI_TRGMOD_POST, WD_AI_TRGSRC_ExtD, WD_AI_TrgPositive, 0, 0.0, totalScans, 0, 0, 0);
    ...
    if (err == NoError) {
        SetCtrlVal(adcTabHandle, TABPANEL_2_ADC_MSG_STATUS, "Diagnostic Burst Captured");
        ...
        char msg[64];
        sprintf(msg, "Sync Read Hardware Error: %d", err);
        SetCtrlVal(adcTabHandle, TABPANEL_2_ADC_MSG_STATUS, msg);
    }

    /* 6. Restore continuous streaming trigger configuration */
    {
        U32 restoreScansPerCh = adcTotalSamples / ADC_NUM_CHANNELS;
        WD_AI_Trig_Config(adcCard, WD_AI_TRGMOD_POST, WD_AI_TRGSRC_ExtD,
                          WD_AI_TrgPositive, 0, 0.0, restoreScansPerCh, 0, 0, 0);
    }
